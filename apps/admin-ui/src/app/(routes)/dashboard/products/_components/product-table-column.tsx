      ...
      ? [
          {
            ...
            header: 'Start Date',
            cell: ({ row }) => {
              const startDate = row.original.startingDate;
              if (!startDate) return '---';
              return <span>{format(new Date(startDate), 'dd/MM/yyyy')}</span>;
            },
          } as ColumnDef<TProductWithImagesAndShop>,
        ]
      : []),
      ...
      ? [
          {
            ...
            header: 'End Date',
            cell: ({ row }) => {
              const endDate = row.original.endingDate;
              if (!endDate) return '---';
              return <span>{format(new Date(endDate), 'dd/MM/yyyy')}</span>;
            },
          } as ColumnDef<TProductWithImagesAndShop>,
        ]
      : []),
