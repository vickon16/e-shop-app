...
import { getProductTableColumn } from '../products/_components/product-table-column';
import { TProductTableRowAction } from '../products/_components/product-table-type';

const AllProductsEventsPage = () => {
  ...
  const columns = useMemo(
    () => getProductTableColumn({ type: 'events', setRowAction }),
    [setRowAction],
  );

  const shopProductEventsQuery = useQuery(getAdminProductEvents());
  const shopProducts = shopProductEventsQuery.data;
  ...
  const handleDeleteRestoreProduct = async (payload: {
    ...
    try {
      ...
      setRowAction(null);
      shopProductEventsQuery.refetch();
    } catch (error) {
    ...
    <>
      <div className="w-full min-h-screen dark p-8">
        ...
        <div className="flex justify-between w-full items-center mb-1 gap-3">
          <h2 className="text-2xl font-semibold">All Product Events</h2>
        </div>
        ...
        <DataTable
          key={tableKey}
          table={table}
          isLoadingTable={shopProductEventsQuery.isLoading}
        >
          <div className="flex items-center justify-end">
            <Button
              ...
              variant="secondary"
              title="refresh table"
              onClick={() => shopProductEventsQuery.refetch()}
            >
              <LuRefreshCw
                className={cn(
                  shopProductEventsQuery.isRefetching && 'animate-spin',
                )}
              />
            </Button>
          </div>
...
export default AllProductsEventsPage;
